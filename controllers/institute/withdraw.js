...
class WithdrawController {
    ...
    async withdrawRequest(req, res) {
        try {
            ...
            let getTotalSales = await CourseOrderServices.getCourseSales(criteria);
            ...
            let criteria2 = {
                withdraw_owner: data.institute_id,
                // status: 2
            }
    ...
    async getWithdrawHistory(req, res) {
        try {
            let data = Object.assign({}, req.body)
            if (!data.institute_id) throw new apiError.ValidationError('Institute id is required', message.REQUIRED);
            let criteria = {
                withdraw_owner: data.institute_id
            }
